                    ...
                    if manifest is not None:
                        ...
                        entry["outputs"] = {
                            n: {
                                ...
                                "dim_labels_from_input": s.dim_labels_from_input,
                                "dim_labels_drop_outer": s.dim_labels_drop_outer,
...
class JobsStore:
        ...
        async def _write(conn: aiosqlite.Connection) -> None:
            await conn.execute(
                ...
                (
                    ...
                    job.started_ts,
                    job.expected_shards,
                    json.dumps(job.shard_element_ids) if job.shard_element_ids else None,
                ),
            )
            await conn.execute(
    ...
    async def get(self, job_id: str) -> Job | None:  # noqa: F821
        ...
        return Job(
            ...
            shard_element_ids=json.loads(row[22]) if row[22] else None,
        )
    ...
    async def list_shards_of(self, parent_job_id: str) -> list[Job]:  # noqa: F821
        ...
        return [
            Job(
                ...
                shard_element_ids=json.loads(r[22]) if r[22] else None,
            )
            for r in rows
        ]
        ...
        return [
            {
                ...
                "shard_element_ids": json.loads(r[16]) if r[16] else None,
            }
            for r in rows
        ]
        ...
        return [
            {
                ...
                "shard_element_ids": json.loads(r[21]) if r[21] else None,
                "created_ts": r[14],
                "updated_ts": r[15],
            }
            for r in rows
